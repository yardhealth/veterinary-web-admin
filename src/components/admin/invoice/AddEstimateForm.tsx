...
const AddEstimateForm = () => {
  ...
  const AddEstimateSchema = useMemo(() => {
    return [
      ...
      },
      {
        ...
        styleContact: 'rounded-xl overflow-hidden bg-white ',
        type: 'text',
        validationSchema: Yup.string().required('Estimate  required'),
        initialValues: '',
        icon: <ReceiptIcon />,
        required: true,
        contactField: {
...

      {
        ...
        options: staffs
          ?.filter((staff: any) => staff?.role === 'sales-person')
      ...
      },
      {
        ...
        type: 'date',
        initialValues: '',
        validationSchema: Yup.string().required('Estimate date required'),
        icon: <ReceiptIcon />,
        required: true,
        contactField: {
          xs: 12,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: any) => {
      ...
      const ID = Date.now()
      try {
        database.ref(`Estimates/${ID}`).update({
          ...values,
          netPrice: subTotal || 0,
        ...
        database.ref(`Customers/${values?.user}/Estimates/${ID}`).update({
          ...values,
          netPrice: subTotal || 0,
          discountPrice: discountSubTotal || 0,
          taxPrice: taxTotal || 0,
